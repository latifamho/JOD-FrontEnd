...
export function RegisterForm() {
  ...
  function validatePhaseOne(): RegisterFieldErrors {
    ...
    if (values.password.length < 8) next.password = "كلمة المرور يجب أن تكون 8 أحرف على الأقل.";
    if (values.password !== values.passwordConfirmation) next.passwordConfirmation = "تأكيد كلمة المرور غير متطابق.";
    return next;
  }
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const validationErrors = { ...validatePhaseOne(), ...validatePhaseTwo() };
    if (Object.keys(validationErrors).length > 0) {
      setErrors(validationErrors);
      if (Object.keys(validationErrors).some((field) => phaseOneFields.has(field as RegisterFieldName))) setPhase("phase-1");
      return;
    }
    ...
    registerMutation.mutate(
      ...
      },
      {
        onError: (error) => {
          ...
          const normalized = normalizeApiError<RegisterFieldName>(error, { fieldAliases: registrationFieldAliases });
          setErrors({ ...normalized.fieldErrors, root: normalized.message });
          if (Object.keys(normalized.fieldErrors).some((field) => phaseOneFields.has(field as RegisterFieldName))) setPhase("phase-1");
        },
      },
    );
  }
  ...
  return (
    <div dir="rtl" className="w-full max-w-2xl text-right md:flex md:h-full md:min-h-0 md:max-h-full md:flex-col md:overflow-hidden">
      <div className="space-y-2">
        ...
        <p className="text-sm leading-6 text-muted-foreground">خطوتان بسيطتان تفصلان منظمتك عن تقديم طلب الانضمام والمراجعة.</p>
      </div>
      <form noValidate onSubmit={handleSubmit} className="mt-6 space-y-5 md:flex md:min-h-0 md:flex-1 md:flex-col md:overflow-hidden">
        <Tabs dir="rtl" value={phase} onValueChange={(value) => {
          ...
          <div className="md:min-h-0 md:flex-1 md:overflow-y-auto md:px-1">
            <TabsContent value="phase-1"><RegisterPhaseOneFields values={values} errors={errors} disabled={registerMutation.isPending} onInputChange={updateValue} /></TabsContent>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4">
          <p className="text-xs text-muted-foreground">لديك حساب بالفعل؟ <Link href="/login" className="font-semibold text-primary hover:underline">تسجيل الدخول</Link></p>
