  ...
  return (
    <section className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="رقم الموبايل الرسمي" error={errors.companyPhone}>
          <div dir="ltr" className={`flex h-11 items-center overflow-hidden rounded-xl border bg-background/85 ${errors.companyPhone ? "border-destructive" : "border-input"}`}>
            ...
            <Input id="companyPhone" name="companyPhone" type="tel" inputMode="numeric" maxLength={9} value={values.companyPhone.replace(/^\+963/, "")} onChange={(event) => onCompanyPhoneChange(event.target.value)} disabled={disabled} aria-invalid={Boolean(errors.companyPhone)} placeholder="9XXXXXXXX" className="h-full rounded-none border-0 bg-transparent text-left shadow-none focus-visible:ring-0" />
          </div>
        </Field>
