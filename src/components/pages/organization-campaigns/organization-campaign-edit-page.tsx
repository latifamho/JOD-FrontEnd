      ...
      <form noValidate className="space-y-5 rounded-xl border bg-card p-4 sm:p-6" onSubmit={handleSubmit(async (values) => {
        try {
          await updateMutation.mutateAsync({ campaignId: campaign.id, body: {
            title: values.title.trim(), summary: values.summary.trim(), categoryId: values.categoryId, audience: values.audience, status: values.status,
        ...
        <Field label="الفئة المستهدفة" error={errors.audience?.message}><Controller control={control} name="audience" render={({ field }) => <Select value={field.value} onValueChange={field.onChange} disabled={isBusy}><SelectTrigger aria-invalid={Boolean(errors.audience)}><SelectValue placeholder="اختر الفئة المستهدفة" /></SelectTrigger><SelectContent>{Object.entries(organizationCampaignAudienceLabels).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select>} /></Field>
