...
const campaignFormSchema = z
  .object({
    title: z
      ...
      .max(255, "عنوان الحملة يجب ألا يتجاوز 255 حرفًا")
      .refine((value) => value.trim().length > 0, "عنوان الحملة مطلوب"),
    summary: z
      .string()
      .min(1, "ملخص الحملة مطلوب")
      .refine((value) => value.trim().length > 0, "ملخص الحملة مطلوب"),
    category: z.enum(["health", "education", "food", "shelter", "employment"]),
  ...
  .superRefine((values, context) => {
    if (values.startDate && values.startDate < getLocalDateInputValue()) {
      context.addIssue({
        code: "custom",
        path: ["startDate"],
        message: "تاريخ البداية لا يمكن أن يكون قبل تاريخ اليوم",
      });
    }
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    step={1}
                    disabled={isSubmitting}
                    aria-invalid={Boolean(errors.beneficiariesCount)}
                    {...register("beneficiariesCount")}
                  />
                  {errors.beneficiariesCount ? (
                    <p className="text-xs text-destructive">
                      {errors.beneficiariesCount.message}
                    </p>
                  ) : null}
                </div>
              </div>
