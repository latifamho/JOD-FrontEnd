...
export function OrganizationAnalyticsPage({ mode }: { mode: 'overview' | 'content' }) {
  ...
  return <section className="space-y-5">
    ...
    {mode === 'content' ? <div className="overflow-x-auto rounded-xl border bg-card"><Table><TableHeader><TableRow><TableHead>المحتوى</TableHead><TableHead>النوع</TableHead><TableHead>التصنيف</TableHead><TableHead>الظهور</TableHead><TableHead>الفتح</TableHead><TableHead>الحفظ</TableHead><TableHead>الإجراءات</TableHead><TableHead>التحويل</TableHead></TableRow></TableHeader><TableBody>{rows.map((row) => <TableRow key={`${row.contentType}-${row.id}`} onClick={() => setSelectedRow(row)} className="cursor-pointer"><TableCell className="font-medium">{row.title || row.id}</TableCell><TableCell>{row.postType || row.contentType}</TableCell><TableCell>{categoryNames.get(row.categoryId ?? '') ?? '-'}</TableCell><TableCell>{row.impressions}</TableCell><TableCell>{row.opens}</TableCell><TableCell>{row.saves}</TableCell><TableCell>{row.actions}</TableCell><TableCell>{row.conversionRate}%</TableCell></TableRow>)}</TableBody></Table></div> : null}
