...
export function CapabilitiesManagementPage() {
  ...
  return <section className="space-y-4">
    ...
    <div className="overflow-auto rounded-xl border"><Table><TableHeader><TableRow><TableHead>الاسم</TableHead><TableHead>المعرف</TableHead><TableHead>الحالة</TableHead><TableHead>عدد المستخدمين</TableHead><TableHead>الترتيب</TableHead><TableHead>الإجراءات</TableHead></TableRow></TableHeader><TableBody>{query.isLoading?Array.from({length:5}).map((_,i)=><TableRow key={i}>{Array.from({length:6}).map((__,j)=><TableCell key={j}><div className="h-4 animate-pulse rounded bg-muted"/></TableCell>)}</TableRow>):query.data?.data.length?query.data.data.map(row=><TableRow key={row.id}><TableCell className="font-medium">{row.name}</TableCell><TableCell>{row.slug}</TableCell><TableCell><Badge variant="outline">{row.status==='active'?'نشط':'غير نشط'}</Badge></TableCell><TableCell>{row.usersCount}</TableCell><TableCell>{row.sortOrder}</TableCell><TableCell><div className="flex gap-2"><Button size="sm" variant="outline" onClick={()=>{setEditing(row);setForm({name:row.name,slug:row.slug,status:row.status,sortOrder:row.sortOrder})}}>تعديل</Button><Button size="sm" variant="outline" disabled={update.isPending} onClick={()=>update.mutate({id:row.id,body:{status:row.status==='active'?'inactive':'active'}})}>{row.status==='active'?'تعطيل':'تفعيل'}</Button></div></TableCell></TableRow>):<TableRow><TableCell colSpan={6} className="py-10 text-center text-muted-foreground">لا توجد طرق مساعدة بعد.</TableCell></TableRow>}</TableBody></Table></div>
