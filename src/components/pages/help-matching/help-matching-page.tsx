...
export function HelpMatchingPage(){
 ...
 <div className="overflow-auto rounded-md border"><Table><TableHeader><TableRow><TableHead>طلب المساعدة</TableHead><TableHead>المساعد</TableHead><TableHead>الحالة</TableHead><TableHead>الاستعجال</TableHead><TableHead>إشارات المطابقة</TableHead><TableHead>العمر</TableHead></TableRow></TableHeader><TableBody>{query.isLoading?Array.from({length:5}).map((_,i)=><TableRow key={i}><TableCell colSpan={6}><div className="h-8 animate-pulse rounded bg-muted"/></TableCell></TableRow>):rows.length?rows.map(row=><TableRow key={row.id} className={row.isStale?'bg-amber-50/60 dark:bg-amber-500/5':''}><TableCell><Link className="font-medium hover:underline" href={`/dashboard/admin/help-matching/${row.id}`}>{row.request?.title??'-'}</Link><p className="text-xs text-muted-foreground">{row.request?.location??'-'}</p></TableCell><TableCell><p>{row.helper?.name??'-'}</p><p className="text-xs text-muted-foreground">{row.helper?.preferredCity??'-'}</p></TableCell><TableCell><Badge variant="outline">{row.status}</Badge>{row.isStale?<Badge className="ms-2" variant="destructive">متأخر</Badge>:null}</TableCell><TableCell>{row.request?.urgency??'-'}</TableCell><TableCell className="text-xs">صريح {row.signals.explicitCategoryWeight} / سلوكي {row.signals.behavioralCategoryWeight}</TableCell><TableCell>{row.ageHours} ساعة</TableCell></TableRow>):<TableRow><TableCell colSpan={6} className="py-10 text-center text-muted-foreground">لا توجد مطابقات.</TableCell></TableRow>}</TableBody></Table></div>
