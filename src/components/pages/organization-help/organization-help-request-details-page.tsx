...
function OfferActions({ offer }: { offer: OrgHelpOfferItem }) {
  ...
  return <div className="flex flex-wrap gap-1">
    ...
    {offer.can.agree ? <Button size="sm" variant="outline" onClick={()=>run('agree')} disabled={action.isPending}>تأكيد الاتفاق</Button> : null}
...
export function OrganizationHelpRequestDetailsPage({ id }: { id: string }) {
  ...
  return <section className="space-y-5">
    ...
    <div><h3 className="mb-3 font-semibold">العروض الواردة</h3>{offersQuery.isLoading ? <DetailsLoadingSkeleton /> : offers.length === 0 ? <EmptyState icon="donors" title="لم تصل أي عروض مساعدة حتى الآن" description="ستظهر العروض الجديدة هنا." /> : <div className="overflow-x-auto rounded-xl border bg-card"><Table><TableHeader><TableRow><TableHead className="w-12">#</TableHead><TableHead>المساعد</TableHead><TableHead>النوع</TableHead><TableHead>الرسالة</TableHead><TableHead>الحالة</TableHead><TableHead>التاريخ</TableHead><TableHead>الإجراءات</TableHead></TableRow></TableHeader><TableBody>{offers.map((offer,index)=><TableRow key={offer.id}><TableCell className="text-muted-foreground">{index+1}</TableCell><TableCell>{offer.helper.name ?? '-'}</TableCell><TableCell>{offer.type ?? '-'}</TableCell><TableCell className="max-w-xs">{offer.description ?? '-'}</TableCell><TableCell><div className="space-y-1"><div className="flex flex-wrap gap-1"><Badge variant="outline">{offerLabels[offer.status] ?? offer.status}</Badge>{offer.isSelectedFinalOffer ? <Badge>العرض النهائي</Badge> : null}</div>{offer.cancelReason ? <p className="max-w-xs text-xs text-destructive">سبب الإلغاء: {offer.cancelReason}</p> : null}{offer.rejectionReason ? <p className="max-w-xs text-xs text-destructive">سبب الرفض: {offer.rejectionReason}</p> : null}</div></TableCell><TableCell>{offer.createdAt ? new Date(offer.createdAt).toLocaleDateString('ar-SY') : '-'}</TableCell><TableCell><OfferActions offer={offer} /></TableCell></TableRow>)}</TableBody></Table></div>}</div>
