            ...
            return (
              <article key={stat.id} className="rounded-xl border bg-card p-4 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm text-muted-foreground">{labelMap[stat.id] ?? stat.label}</p>
        ...
        {overview.isLoading ? (
          ...
          <div className="mt-3">
            <EmptyState
              ...
              description="ستظهر هنا آخر النشاطات المسجلة في المنظمة."
            />
          </div>
        ) : (
            ...
            {activity.map((item) => (
              ...
              >
                ...
                <time className="text-xs text-muted-foreground">
                  {formatUtcDateTimeOrDash(item.at ?? undefined)}
                </time>
              </li>
            ))}
          </ul>
