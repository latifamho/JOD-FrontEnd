  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        dir="rtl"
        className="w-[95vw] border-border p-0 sm:max-w-3xl"
      >
        ...
        <div className="space-y-4 overflow-y-auto p-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="outline">{campaignCategoryLabels[campaign.category]}</Badge>
          ...
          <div className="rounded-lg border border-border bg-muted/40 p-4">
            ...
            <div className="mt-2 grid gap-2 text-xs text-muted-foreground sm:grid-cols-2">
              <p>
                الهدف: <span className="font-semibold text-foreground">{formatAmount(campaign.goalAmount)} ر.س</span>
              </p>
              <p>
                المحصل: <span className="font-semibold text-foreground">{formatAmount(campaign.raisedAmount)} ر.س</span>
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-3 rounded-lg border border-border bg-muted/40 p-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="text-sm font-medium">{campaign.managerName}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-sm font-medium">{formatUtcDate(campaign.startDate)}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">تاريخ النهاية</p>
              <p className="text-sm font-medium">{formatUtcDate(campaign.endDate)}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">تاريخ الإرسال للمراجعة</p>
              <p className="text-sm font-medium">{formatUtcDateTime(campaign.submittedAt)}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">آخر مراجع</p>
              <p className="text-sm font-medium">{campaign.reviewedBy ?? "—"}</p>
            </div>
          </div>
